...
namespace NeoCource.Editor.Quizzes
{
    ...
    {
        private static readonly Regex QuizBlockRegex = new(
            "```quiz\\s*\\n([\\s\\S]*?)\\n```",
        ...
        public static List<QuizQuestion> ParseQuestions(string markdown)
        {
            ...
            foreach (Match m in QuizBlockRegex.Matches(markdown))
            {
                ...
                QuizQuestion q = ParseSingleQuestion(body);
                if (q != null)
                {
                    result.Add(q);
                }
        ...
        private static QuizQuestion ParseSingleQuestion(string body)
        {
            ...
            while (i < lines.Length)
            {
                ...
                if (StartsWithKey(line, "kind:"))
                {
                    kindStr = GetValue(line);
                    i++;
                    continue;
                }
                ...
                if (StartsWithKey(line, "answers:"))
                {
                    ...
                    while (i < lines.Length && lines[i].TrimStart().StartsWith("- "))
                    {
                        ...
                        if (StartsWithKey(aLine, "text:"))
                        {
                            aText = GetValue(aLine);
                        }

                        if (StartsWithKey(aLine, "correct:"))
                        {
                            aCorrect = IsTrue(GetValue(aLine));
                        }
                        ...
                        while (j < lines.Length && lines[j].StartsWith("    "))
                        {
                            ...
                            if (StartsWithKey(cont, "text:"))
                            {
                                aText = GetValue(cont);
                            }
                            else if (StartsWithKey(cont, "correct:"))
                            {
                                aCorrect = IsTrue(GetValue(cont));
                            }
            ...
            if (string.IsNullOrEmpty(id) || string.IsNullOrEmpty(kindStr) || string.IsNullOrEmpty(text) ||
                answers.Count == 0)
            {
                return null;
            }
            ...
            QuizQuestion q = new()
            {
                ...
                text = text,
                answers = answers.Select((a, idx) => new QuizAnswer
                    { id = "a" + idx, text = a.text, isCorrect = a.correct }).ToList()
            };
            return q;
        }
