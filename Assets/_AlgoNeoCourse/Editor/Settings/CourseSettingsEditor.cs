...
namespace NeoCource.Editor.Settings
{
    ...
    public class CourseSettingsEditor : UnityEditor.Editor
    {
        ...
        {
            var settings = (CourseSettings)target;
        ...
        private void DrawSourceSection(CourseSettings settings)
        {
            ...
            EditorGUILayout.PropertyField(courseJsonFileProperty);
        ...
        private void DrawLessonsSection(CourseSettings settings)
        {
            ...
            {
                foreach (var item in settings.lessonSelections)
                {
        ...
        private void DrawDangerSection(CourseSettings settings)
        {
            ...
            AlgoNeoEditorGui.DrawInfoBox("Сброс вернёт базовые пути и очистит список уроков, но не затронет локальный прогресс квизов.", MessageType.Warning);
