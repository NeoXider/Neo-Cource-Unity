...
namespace Markdig.Syntax
{
    ...
    public class LinkReferenceDefinition : LeafBlock
    {
        ...
        public static bool TryParse<T>(ref T text, [NotNullWhen(true)] out LinkReferenceDefinition? block)
            where T : ICharIterator
        {
            block = null;

            int startSpan = text.Start;

            if (!LinkHelper.TryParseLinkReferenceDefinition(ref text, out string? label, out string? url,
