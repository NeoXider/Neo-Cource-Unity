...
namespace Markdig.Parsers
{
    ...
    public class HtmlBlockParser : BlockParser
    {
        ...
        {
            BlockState result = MatchStart(processor);
        ...
        private BlockState MatchStart(BlockProcessor state)
        {
            ...
            line.SkipChar();
            BlockState result = TryParseTagType16(state, line, state.ColumnBeforeIndent, startPosition);
        ...
        private BlockState TryParseTagType16(BlockProcessor state, StringSlice line, int startColumn, int startPosition)
        {
            ...
            if (!HtmlTags.TryMatchExact(tag.Slice(0, count), out KeyValuePair<string, int> match))
            {
                return BlockState.None;
            }
