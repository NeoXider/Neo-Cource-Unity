...
namespace Markdig.Extensions.Emoji
{
    ...
    public class EmojiMapping
    {
        ...
        /// </summary>
        public static readonly EmojiMapping DefaultEmojisAndSmileysMapping = new();

        /// <summary>
        ///     The default emoji shortcodes mapping, without smileys.
        /// </summary>
        public static readonly EmojiMapping DefaultEmojisOnlyMapping = new(false);
        ...
        /// <summary>
        ///     Constructs a mapping from a dictionary of emoji shortcodes to unicode, and a dictionary of smileys to emoji
        ///     shortcodes.
        /// </summary>
        public EmojiMapping(IDictionary<string, string> shortcodeToUnicode,
            IDictionary<string, string> smileyToShortcode)
        {
            ...
            foreach (KeyValuePair<string, string> shortcode in shortcodeToUnicode)
            {
                if (string.IsNullOrEmpty(shortcode.Key) || string.IsNullOrEmpty(shortcode.Value))
                {
                    ThrowHelper.ArgumentException("The dictionaries cannot contain null or empty keys/values",
                        nameof(shortcodeToUnicode));
                }
            ...
            foreach (KeyValuePair<string, string> smiley in smileyToShortcode)
            {
                if (string.IsNullOrEmpty(smiley.Key) || string.IsNullOrEmpty(smiley.Value))
                {
                    ThrowHelper.ArgumentException("The dictionaries cannot contain null or empty keys/values",
                        nameof(smileyToShortcode));
                }

                if (!shortcodeToUnicode.TryGetValue(smiley.Value, out string? unicode))
                {
                    ThrowHelper.ArgumentException(string.Format(
                        "Invalid smiley target: {0} is not present in the emoji shortcodes dictionary", smiley.Value));
                }
                ...
                {
                    ThrowHelper.ArgumentException(string.Format("Smiley {0} is already present in the emoji mapping",
                        smiley.Key));
                }
            }
