    ...
    public class HostProviderBuilder
    {
        internal static Dictionary<string, IHostProvider> KnownHosts { get; }
            = new(StringComparer.OrdinalIgnoreCase)
            {
                ...
                ["Yandex"] = Create("music.yandex.ru", Yandex, false, "yandex"),
        ...
        /// <param name="handler">
        ///     Handler that generate iframe url, if uri cannot be handled, it can return <see langword="null" />
        ///     .
        /// </param>
        ...
        public static IHostProvider Create(string hostPrefix, Func<Uri, string?> handler, bool allowFullScreen = true,
        ...
        {
            public DelegateProvider(string hostPrefix, Func<Uri, string?> handler, bool allowFullscreen = true,
        ...
        {
            string query = uri.Query.Substring(uri.Query.IndexOf('?') + 1);
        ...
        private static string? YouTube(Uri uri)
        {
            ...
            if (string.Equals(uriPath, "/embed", StringComparison.OrdinalIgnoreCase) ||
                uriPath.StartsWith("/embed/", StringComparison.OrdinalIgnoreCase))
            {
                return uri.ToString();
            }

            if (!string.Equals(uriPath, "/watch", StringComparison.OrdinalIgnoreCase) &&
                !uriPath.StartsWith("/watch/", StringComparison.OrdinalIgnoreCase))
            {
                return null;
            }

            string[] queryParams = SplitQuery(uri);
