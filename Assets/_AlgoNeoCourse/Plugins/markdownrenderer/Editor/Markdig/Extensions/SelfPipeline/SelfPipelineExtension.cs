...
namespace Markdig.Extensions.SelfPipeline
{
    ...
    public sealed class SelfPipelineExtension : IMarkdownExtension
    {
        ...
        public MarkdownPipeline CreatePipelineFromInput(string inputText)
        {
            ...
            int indexOfSelfPipeline = inputText.IndexOf(SelfPipelineHintTagStart, StringComparison.OrdinalIgnoreCase);
            if (indexOfSelfPipeline >= 0)
            {
                int optionStart = indexOfSelfPipeline + SelfPipelineHintTagStart.Length;
                int endOfTag = inputText.IndexOf("-->", optionStart, StringComparison.OrdinalIgnoreCase);
                if (endOfTag >= 0)
                {
