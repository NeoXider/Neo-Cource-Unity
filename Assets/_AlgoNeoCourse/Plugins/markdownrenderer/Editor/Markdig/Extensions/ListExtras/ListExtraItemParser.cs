...
namespace Markdig.Extensions.ListExtras
{
    ...
    public class ListExtraItemParser : OrderedListItemParser
    {
        ...
        public override bool TryParse(BlockProcessor state, char pendingBulletType, out ListInfo result)
        {
            ...
            bool isRomanLow = CharHelper.IsRomanLetterLowerPartial(c);
            bool isRomanUp = !isRomanLow && CharHelper.IsRomanLetterUpperPartial(c);
                ...
                (pendingBulletType == '\0' || pendingBulletType == 'i' || pendingBulletType == 'I'))
            {
                ...
                do
                {
                    c = state.NextChar();
                             ...
                             ? CharHelper.IsRomanLetterLowerPartial(c)
                             : CharHelper.IsRomanLetterUpperPartial(c));
