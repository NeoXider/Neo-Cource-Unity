...
- выпадающий список уроков с `%` прохождения и галочками `✓` у завершённых;
- полоса общего прогресса курса;
- кнопка `▶ Продолжить` (возврат к сохранённой позиции);
- навигация по слайдам (только кнопки `<` / `>` и стрелки; `unity://slide`-ссылки удалены в `1.6.0`);
...
Сохранения строго per-project: путь вне текущего проекта игнорируется и не перезаписывает
локальную сессию. Личные `Progress/`, `QuizState/`, `VideoCache/` не коммитятся (в `.gitignore`).

Ручной перенос: `Tools -> AlgoNeoCourse -> Progress -> Export/Import JSON...`.

Проверка уроков: `Tools -> AlgoNeoCourse -> Validate Lessons` — дубли `id:`, пустые `answers`,
неизвестные `kind`, битые медиа и `unity://`-ссылки списком с переходом к файлу.
...
- fenced code blocks;
- `quiz`;
- `check`-блоки (кнопка «Проверить» подставляется автоматически);
- ссылки `unity://open`;
- изображения и видео.
...
- Прогресс курса: полоса `%`, галочки `✓` и проценты в списке уроков, кнопка `▶ Продолжить`.
- `Tools -> AlgoNeoCourse -> Validate Lessons`: панель ошибок `.md` (дубли `id:`, пустые `answers`, битые медиа/ссылки).
- `Tools -> AlgoNeoCourse -> Progress -> Export/Import JSON...`: ручной перенос прогресса.
- Анимации: «поп» верного ответа в квизах, появление/«толчок» результата автопроверки.
- `unity://slide`-ссылки удалены (навигация — кнопки и стрелки); кнопка `>` уважает guard квизов.
- GIF-конвертация в фоне (без фризов UI), атомарная запись прогресса, per-project изоляция сейвов.
- Поддержка Unity 6.0–6.6 (`UxmlElement`, без legacy `UxmlFactory`).

### Что нового в 1.6.1

- Кнопка «Проверить» красится в зелёный/красный по итогу автопроверки.
- `check`-блоки — основной механизм практик: кнопки подставляются всегда, результаты идут в прогресс (`слайд · квизы · проверки`).
- Авторские `unity://check`-ссылки удалены (транспорт остался внутренним для кнопок); валидатор ругается на них ошибкой.
- Прогресс-бар текущего урока: доля от всех элементов + цвет (оранжевый → жёлтый → зелёный, 100% — синий).
- Кнопка `▶ Продолжить` ведёт к дальней достигнутой точке (откат назад её не сдвигает).
- Автообновление окна курса после скачивания уроков.
- Компактная двухрядная шапка, порядок в меню `Tools/AlgoNeoCourse`, заголовок окна не слетает.
